...
import com.example.client.dto.MeteodataDTO;
import com.example.client.dto.MeteodatasResponse;
...
public class DrawClient {
	...
	private static List<Double> getTemperaturesFromServer() {
		...
		final String url = "http://localhost:8080/meteodata";

		MeteodatasResponse jsonResponse = restTemplate.getForObject(url, MeteodatasResponse.class);

		if (jsonResponse == null || jsonResponse.getAllMeteodata() == null)
			return Collections.emptyList();

		return jsonResponse.getAllMeteodata().stream().map(MeteodataDTO :: getTemperature)
